...
#Run 20 simulations
while (i <= 20) {
    #Generate 120 observations with p = 300 features each
    x <- matrix(rnorm(120*p), ncol = p)
    #Change mean from each cluster for the differing features
    x[1:20, 1:75] <- x[1:20, 1:75] + 2
    x[21:40, 1:75] <- x[21:40, 1:75] + 1.2
    x[41:60, 1:75] <- x[41:60, 1:75] + 0.4
    x[61:80, 1:75] <- x[61:80, 1:75] - 0.4
    x[81:100, 1:75] <- x[81:100, 1:75] -1.2
    x[101:120, 1:75] <- x[101:120, 1:75] - 2
    x <- scale(x, TRUE, TRUE)
    #Generate labels for clusters
    c <- rep(1:6, each = 20)
    dataused <- cbind(c, x)
    #Generate names for the observations
    rownames(dataused) <- seq(1,120,1)
    dataused2 <- x
    
    k <- numeric(21)
    
    #use NbClust to find number of clusters
    for (j in 1:21) {
      method_used <- bro[j]
      output <- NbClust(data = dataused2, diss = NULL, distance = "euclidean", min.nc = 4, max.nc = 8, method = "kmeans", index = method_used)
      k[j] <- output$Best.nc[1]
    }
    
    #Identify number of clusters by majority rule and save value
    ...
    while (j <= 25) {
      ...
      #run cross validation for each fold as validation data once
      for (t in 1:5) {
        #identify and put together the training and validation data based on their fold id
    ...
    for (j in 1:length(cluster_options)) {
      LVR_check <- KMeansSparseCluster.permute(dataused2, j+3,5, best_k_and_s_LVR, silent=TRUE)
      Gaps_LVR[j] <- LVR_check$gaps[j]
      SDR_check <- KMeansSparseCluster.permute(dataused2, j+3, 5, best_k_and_s_SDR, silent=TRUE)
