...
dataused <- as.data.frame(Gas_Data)

#remove all variables but the sensor measured features
dataused2 <- dataused[, -c(1:8)]
...
for (i in 1:21) {
  method_used <- bro[i]
  output <- NbClust(data = dataused2, diss = NULL, distance = "euclidean", min.nc = 10, max.nc = 14, method = "kmeans", index = method_used)
...
#generate sequence of possible values for s in between 1 and the square root of the number of dimensions of dataused2
s_values <- seq(1.1, sqrt(p), 0.5)
i = 1

#execute 100 times the method for selecting the optimal s with the largest value and standard deviation rule in addition to the Altered gap statistic
while (i <= 25) {
...
while (i <= 25) {
  ...
  for (j in 1:5) {
    #change to matrix to prevent algorithm mistake with smaller datasets and define training and validating sample
    validateIndex <- which(datasplit == j)
    datavalidate <- datashuffling[validateIndex,]
    datatrain <- datashuffling[-validateIndex,]
    
    #remove all information but the sensor measured features
    datavalidating <- datavalidate[, -c(1:8)]
    datatraining <- datatrain[, -c(1:8)]
    ...
    CER_list_LVR[j] <- CER(labels_validate_LVR, datavalidate$label, nrow(datavalidate))
    ...
    CER_list_SDR[j] <- CER(labels_validate_SDR, datavalidate$label, nrow(datavalidate))
  }
...
CER_s_after_k_LVR <- (CER(labels_LVR, dataused$label, n))
...
CER_s_after_k_SDR <- (CER(labels_SDR, dataused$label, n))
...
CER_AGap <- (CER(labels_AGap, dataused$label, n))
...
CER_CV_LVR <- (CER(labels_CV_LVR, dataused$label, n))
...
CER_CV_SDR <- CER(labels_CV_SDR, dataused$label, n)


#set range of clusters
cluster_options <- seq(10, 14, 1)
...
#loop 100 times each time shuffling the data and then determining for all possible values of K the optimal value of s according to both rules
while (t <= 25) {
  datashuffled <- dataused2[sample(1:nrow(dataused2)),]
...
for (i in 1:length(cluster_options)) {
  LVR_check <- KMeansSparseCluster.permute(dataused2, i+9,5, best_k_and_s_LVR, silent=TRUE)
  Gaps_LVR[i] <- LVR_check$gaps[i]
  SDR_check <- KMeansSparseCluster.permute(dataused2, i+9, 5, best_k_and_s_SDR, silent=TRUE)
...
jointLVR_CER <- CER(jointLVR_labels, dataused$label, n)
jointSDR_CER <- CER(jointSDR_labels, dataused$label, n)
